...
// Inyectable es un decorador necesario que viene del angular core para poder crear servicios
...
export class HeroesService {
  ...
  getHeroe(idx: string) {
    // esto funcion me muestra el indice del heroe para ser utlizada  en heroe
  ...
  buscarHeroes(termino: string) {
    ...
    // aca barremos cada uno de los heroes del arreglo de arriba para hacer la busqueda,
    for (let i = 0; i < this.heroes.length; i++) {
      let heroe = this.heroes[i];
      let nombre = heroe.nombre.toLowerCase();
      // decimos que si el termino que se paso como parametro coincide con alguno de los nombres del array es pq da 0 o mayor, entonces le hacemos push a ese arreglo temporal, en caso contrario da -1
...
export interface Heroe {
  // este es el tipo de arreglo q queremos q se implemente y solo tenga estas propiedades tipo string, lo podemos exportar para utilziarlo como tipo en otro lado
  nombre: string;
  bio: string;
  img: string;
  aparicion: string;
